    ...
    <section id="usecases" className="py-24 px-6 bg-white">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-14">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-4">Use cases</p>
          ...
          {cases.map(({ icon: Icon, title, persona, body, tags }) => (
            <div key={title} className="card p-7">
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  <h3 className="text-slate-900 font-semibold text-base leading-tight">{title}</h3>
                  <p className="text-xs text-sky-600 font-medium mt-0.5">{persona}</p>
                </div>
              </div>
              <p className="text-slate-500 text-sm leading-relaxed mb-4">{body}</p>
              <div className="flex flex-wrap gap-2">
                {tags.map((tag) => (
                  <span key={tag} className="text-xs text-slate-500 bg-slate-50 border border-slate-200 px-2.5 py-1 rounded-full font-medium">
                    {tag}
                  </span>
                ))}
              </div>

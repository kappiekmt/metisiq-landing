...
export default function DashboardMockup() {
  ...
  return (
    <div ref={ref} className="dashboard-float rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-2xl shadow-slate-200/60 dark:shadow-slate-900/60 overflow-hidden">
      {/* Browser chrome */}
      <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
        ...
        <div className="flex-1 mx-4 h-6 rounded-md bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 flex items-center px-3">
          <span className="text-xs text-slate-400 dark:text-slate-500">app.metisiq.io/dashboard</span>
        </div>
      </div>

      <div className="flex h-[380px] md:h-[420px]">
        {/* Sidebar */}
        <div className="w-52 border-r border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 p-3 hidden md:flex flex-col gap-0.5">
          {['Dashboard', 'Production', 'Energy', 'Machine OEE', 'Linen', 'Cost Price', 'Reports', 'Settings'].map((item, i) => (
            ...
            >
              <div className={`w-1.5 h-1.5 rounded-full ${i === 0 ? 'bg-sky-500' : 'bg-slate-300 dark:bg-slate-600'}`} />
              {item}
            </div>
          ))}
        </div>
        ...
        {/* Main content */}
        <div className="flex-1 p-5 bg-white dark:bg-slate-800 overflow-hidden">
          {/* KPI row */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            {[
              ...
              <div
                key={stat.label}
                className="p-3 rounded-xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900"
                style={{
                  opacity: ready ? 1 : 0,
              ...
              >
                <p className="text-[10px] text-slate-400 dark:text-slate-500 mb-1 font-medium">{stat.label}</p>
                <p className="text-base font-bold text-slate-900 dark:text-white">{stat.value}</p>
                <p className="text-[10px] mt-0.5 text-emerald-600 dark:text-emerald-400 font-medium">{stat.delta} vs last week</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div
              className="md:col-span-2 p-4 rounded-xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900"
              style={{
                opacity: ready ? 1 : 0,
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">Production throughput</p>
                <span className="text-[10px] text-emerald-600 bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-100 dark:border-emerald-900 px-2 py-0.5 rounded-full font-medium flex items-center gap-1">
            ...
            <div
              className="p-4 rounded-xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900"
              style={{
                opacity: ready ? 1 : 0,
                transition: 'opacity 0.6s ease 0.65s',
              }}
            >
              <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-3">Energy split</p>

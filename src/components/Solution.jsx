...
export default function Solution() {
  return (
    <section id="solution" className="py-24 px-6 bg-white dark:bg-slate-900">
      <div className="max-w-5xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          <AnimateIn from="left">
            <p className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4">Our solution</p>
            ...
            <div className="grid grid-cols-2 gap-3">
              {kpis.map((kpi) => (
                <div key={kpi.label} className="card p-5 text-center">
                  ...
                  <p className="text-xs font-semibold text-sky-600 dark:text-sky-400">{kpi.label}</p>
                  <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{kpi.sub}</p>
                </div>
              ))}
              <div className="col-span-2 p-5 rounded-2xl border border-sky-100 dark:border-sky-900/50 bg-sky-50 dark:bg-sky-950/30 text-center">
                <p className="text-sm text-sky-700 dark:text-sky-400 italic font-medium">
